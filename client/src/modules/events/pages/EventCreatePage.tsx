...
export default function EventCreate() {
  ...
  const activeStaffMembers = staffMembers?.filter((s) => s.isActive) || [];

  // For coordinator - filter by COORDINATOR position
  const coordinators = activeStaffMembers.filter(
    (s) => s.position?.toUpperCase() === "COORDINATOR" || s.position?.toUpperCase() === "KOORDINÁTOR"
  );
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-col">
                      ...
                      <Popover open={coordinatorOpen} onOpenChange={setCoordinatorOpen}>
                        ...
                        <PopoverContent className="w-full p-0">
                          <Command>
                            <CommandInput placeholder="Hledat koordinátora..." />
                            <CommandList>
                              <CommandEmpty>Žádný koordinátor nenalezen</CommandEmpty>
                              <CommandGroup>
                                {coordinators.map((staff) => (
                                  <CommandItem
                                    key={staff.id}
                                    value={`${staff.firstName} ${staff.lastName}`}
                                    onSelect={() => {
                      ...
                      <FormDescription>Pouze personál s rolí koordinátor</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
